...
(function($) {
    ...
    $.each($('.tab-pane.plus [data-required="required"]'),function (){
        $(this).prop('required',false);
        $(this).attr('disabled','disabled');
    });
    $('.nav-link.plus').on('click',function (){
    ...
    $('*[data-tab="tabs"]').on('click',function (){
        var lang= $(this).data('lang');
        $('.clpse').removeClass('show');
        $('.clpse.lang-'+lang).addClass('show');
    });
    var initBodyClass = "";
    ...
    var Unifato = {
        ...
        rightSidebarToggle: function() {
            if (!$(".right-sidebar-toggle").length) return;
        ...
        enableFullCalendarEvents: function() {
            ...
            el.each(function(index) {
                ...
                $this.find('.fc-add-event').click(function() {
                    $('#eventAddModal').modal('show');
                    window.ThemeFullCalendar.EventAddListMode = {
                        target: $this
                    };
                });
            });
        ...
        enableDropzone: function() {
            ...
            Dropzone.prototype.defaultOptions.timeout = 1800000;
            var defaults = {
                headers: {
        ...
        enableSlugify: function() {
            var el = $('[data-toggle="slug"]');
            if (!el.length) return;
            el.each(function() {
                var $this = $(this),
                    alias = $this.data('enter-name');
                var myVar = '';
                $this.donetyping(function() {
                    $('[name="' + alias + '"]').val(slugify($this.val())).change();
                });
                $('[name="' + alias + '"]').on('change', function() {
                    var slug = $('[name="' + alias + '"]').val();
                    Pace.ignore(function() {
                        $.get(window.backend_url + '/check-slug/' + slug, function(newSlug) {
                            if (newSlug != slug) {
                                $this.parents('.form-group:first').append("<small class=\"text-muted text-danger\">Aynı başlıkta içerik bulunmaktadır</small>");
                            } else {
                                $this.parents('.form-group:first').find('small.text-muted').remove();
                            }
                            $('[name="' + alias + '"]').val(newSlug);
                        });
                    });
        ...
        enableCKEditor: function() {
            ...
            CKEDITOR.config.contentsCss = 'https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css';
            CKEDITOR.config.mj_variables_bootstrap_css_path = 'https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css';
            CKEDITOR.config.allowedContent = true;
            CKEDITOR.config.bootstrapGrid_container_extra_large = 1140;
            CKEDITOR.config.bootstrapGrid_container_large = 960;
            CKEDITOR.config.bootstrapGrid_container_medium = 720;
            CKEDITOR.config.bootstrapGrid_container_small = 540;
            CKEDITOR.config.bootstrapGrid_grid_columns = 12;
            CKEDITOR.config.removeButtons = 'NewPage,Preview,Print,Save,Form,Checkbox,Radio,TextField,Textarea,Select,Button,ImageButton,HiddenField,CreateDiv,Flash,Smiley,Font,About,SelectAll,CopyFormatting,Language,Templates';
            CKEDITOR.config.filebrowserImageBrowseUrl = window.backend_url + '/media-manager/browse?type=TYPE_IMAGE';
            CKEDITOR.config.filebrowserBrowseUrl = window.backend_url + '/media-manager/browse?type=TYPE_PDF,TYPE_DOCUMENT';
            CKEDITOR.config.allowedContent = true;
        ...
        enableTinyMCE: function() {
            ...
            var defaults = {
                ...
                templates: [{
                    title: 'Form ekle',
                    content: '[form slug="online-randevu" col="6,6,6,6"]'
                },
                    {
                        title: 'Galeri ekle',
                        content: '[gallery slug="anasayfa-slider"]'
                    }
                ],
                style_formats: [{
                    title: 'Headers',
                ...
                plugins: 'fullscreen image link media table hr insertdatetime lists imagetools code template',
                toolbar1: 'paste pastetext | template | formatselect | bold italic strikethrough | link image media |alignleft aligncenter alignright alignjustify  | numlist bullist outdent indent  | removeformat | code |',
                file_browser_callback: ruzgarFileBrowser,
                image_advtab: true
            };
            el.each(function() {
                var $this = $(this),
    ...
    window.addMedia = function(mediaUrl, mediaId, element) {
        ...
        if (parentElement.length > 0) {
            ...
            /*parentElement.find('.alt-input').attr('required',true);*/
            $(element).fadeOut();
        } else {
    ...
    // $('.img-select input:not(.alt-input)').on('change',function(){
    //   var mediaInput = $(this).parents('.img-select').find('[data-role="file-browser"]').data('media');
    //   $(this).parents('.img-select').append('<button type="button" class="btn-reset btn-danger btn btn-xs btn-block" onclick="window.resetMedia(\'#'+mediaInput+'\');">Kaldır</button>');
    //   $(this).parents('.img-select').find('.alt-input').fadeIn();
    ...
    window.fileBrowser = function(element) {
        ...
        var mediaUrl = window.backend_url + '/media-manager/browse?type='+type+'&input=' + $(element).data('media')+'&contentTitle='+$('[data-toggle="slug"]').eq(0).val();
    ...
    var topSettingForm = $('#top-setting form');
    $('#top-setting form *').on('change', function() {
        topSettingForm.submit();
    });

    topSettingForm.on('submit', function() {
        $.ajax({
            type: "POST",
            url: topSettingForm.attr('action'),
            dataType: 'json',
            data: topSettingForm.serialize(),
...
function ruzgarFileBrowser(field_name, url, type, win) {
    var ruzgarFileman = window.backend_url + '/media-manager/browse';
    if (ruzgarFileman.indexOf("?") < 0) {
        ruzgarFileman += "?type=" + type;
    } else {
        ruzgarFileman += "&type=" + type;
    }
    ruzgarFileman += '&input=' + field_name + '&value=' + win.document.getElementById(field_name).value;
    if (tinyMCE.activeEditor.settings.language) {
        ruzgarFileman += '&langCode=' + tinyMCE.activeEditor.settings.language;
    }
    tinyMCE.activeEditor.windowManager.open({
        file: ruzgarFileman,
        title: 'Ortam Yöneticisi',
        width: 950,
        height: 650,
        resizable: "yes",
        plugins: "media",
...
if (document.addEventListener) {
    document.addEventListener('invalid', function(e) {
        ...
        $.toast({
            text: 'Hatalı alanlar kırmızı olarak işaretlenmiştir.',
            icon: 'error',
            position: 'top-right',
            stack: false
        });
...
$('[data-toggle="language-collapse"]').on('click',function(){
    $('[data-toggle="language-collapse"]').removeClass('active');
    $('[class*="lang-"]').hide();
